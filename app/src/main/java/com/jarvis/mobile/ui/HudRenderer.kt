...
/**
 * Stand-alone renderer for the J.A.R.V.I.S HUD. Drives its own animation state
 * and exposes [draw] so both [HudView] and the live-wallpaper engine can use it.
 */
class HudRenderer {
    ...
    /** Advance the animation one frame. Call before [draw]. */
    fun advance() {
        tick++
    ...
    /** Draw the HUD. [bottomReserve] = pixels at the bottom that should NOT host the core (for overlay panels). */
    fun draw(canvas: Canvas, w: Int, h: Int, bottomReserve: Int = 0) {
        val wf = w.toFloat(); val hf = h.toFloat()
        val usableH = (hf - bottomReserve).coerceAtLeast(hf * 0.5f)
        cx = wf / 2f; cy = usableH * 0.50f
        baseR = min(wf, usableH) * 0.30f
        val col = colorForState()
        val (r, g, b) = decompose(col)

        drawGrid(canvas, wf, hf, r, g, b)
        drawCornerBrackets(canvas, wf, hf, col, r, g, b)
    ...
    private fun drawCornerBrackets(canvas: Canvas, w: Float, h: Float, col: Int, r: Int, g: Int, b: Int) {
        ...
        canvas.drawLine(pad, pad + len, pad, pad, pStroke); canvas.drawLine(pad, pad, pad + len, pad, pStroke)
        canvas.drawLine(w - pad - len, pad, w - pad, pad, pStroke); canvas.drawLine(w - pad, pad, w - pad, pad + len, pStroke)
        canvas.drawLine(pad, h - pad - len, pad, h - pad, pStroke); canvas.drawLine(pad, h - pad, pad + len, h - pad, pStroke)
        canvas.drawLine(w - pad - len, h - pad, w - pad, h - pad, pStroke); canvas.drawLine(w - pad, h - pad, w - pad, h - pad - len, pStroke)
        pFill.color = col
...
/** Shared global state — read by HudView (in-app) and the live wallpaper. */
